...
namespace Microsoft.Extensions.Diagnostics.Tracing
{
    internal sealed class DefaultActivitySourceFactory : ActivitySourceFactory
    {
        ...
        public DefaultActivitySourceFactory(IEnumerable<ActivityListenerBuilder> listenerBuilders, IOptionsMonitor<TracingOptions> options)
        {
            ...
            try
            {
                foreach (ActivityListenerBuilder listenerBuilder in listenerBuilders)
                {
                    registrations.Add(new ActivityListenerRegistration(listenerBuilder, this, initial));
                }
        ...
        private void UpdateRules(TracingOptions options)
        {
            ...
            // The wrapper listener's ShouldListenTo predicate is owned by the registration itself
            // (no user callback runs in the rule-update path), so UpdateRules cannot surface user
            // code. Any throw here would be a bug in our own rule-evaluation code and should
            // propagate to the OnChange caller so it is not silently swallowed.
        ...
        private sealed class ActivityListenerRegistration : IDisposable
        {
            ...
            public ActivityListenerRegistration(ActivityListenerBuilder listenerBuilder, DefaultActivitySourceFactory activitySourceFactory, TracingOptions options)
            {
                ArgumentNullException.ThrowIfNull(listenerBuilder);
                ...
                // Snapshot the user-supplied delegates so subsequent mutation of the builder cannot
                // change the registered listener's behaviour, and so the wrapper methods do not
                // perform a live field read on every callback.
            ...
            public void UpdateRules(List<TracingRule> rules)
            {
                ...
                // wrapper's ShouldListenTo for every active source. ShouldListenTo runs only our
                // own rule-evaluation code (no user callback), so there is no risk of lock-order
                // inversion against user code. Keeping the call outside _lock still avoids holding
                // our lock while the BCL acquires its internal listener registry locks.
            ...
            private ActivitySamplingResult WrappedSample(ref ActivityCreationOptions<ActivityContext> options)
            {
                ...
                return _sample?.Invoke(ref options) ?? ActivitySamplingResult.None;
            }
            ...
            private ActivitySamplingResult WrappedSampleUsingParentId(ref ActivityCreationOptions<string> options)
            {
                ...
                return _sampleUsingParentId?.Invoke(ref options) ?? ActivitySamplingResult.None;
            }
